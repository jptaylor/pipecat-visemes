...
"""pipecat-visemes - Pipecat Voice Agent with server-side lipsync

The ``pipecat init quickstart`` cascade bot (Speech-to-Text → LLM →
Text-to-Speech) with ``LipsyncProcessor`` and ``LipsyncMessageRelay`` wired
into the pipeline; see ``lipsync/`` and the README.
