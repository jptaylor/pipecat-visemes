...
def _add(syms, cls, o, w, r):
    for s in syms.split(): T[s] = (cls, o, w, r)
...
def lookup(sym):
    ...
    if sym in ("", "_", " "): return SIL
    # strip length/stress marks and retry
    base = sym.replace("ː", "").replace("ˈ", "").replace("ˌ", "")
    if base in T: return T[base]
    for k in T:
        if sym.startswith(k): return T[k]
    return None

async def analyze(pcm16k):
    a = FormantLipsyncAnalyzer(collect_debug=True); await a.start(16000)
    ctx = LipsyncAnalysisContext("x", 16000)
    kfs, evs = [], []
    for i in range(0, len(pcm16k), 320):
        r = await a.analyze(pcm16k[i:i+320], ctx); kfs += r.keyframes; evs += r.events
    r = await a.flush(ctx); kfs += r.keyframes; evs += r.events
    return kfs, evs, a.debug_features

def score(kfs, evs, debug, segs):
    offs = np.array([d.offset for d in debug])
    n = len(offs)
    cls = [None]*n; tgt = np.zeros((n, 3))
    unknown = set()
    for i, t in enumerate(offs):
        sym = "_"
        for s, e, p in segs:
            if s <= t < e: sym = p; break
        row = lookup(sym)
        if row is None: unknown.add(sym); row = ("NEUTRAL_C", 0.25, 0.5, 0.1)
        cls[i] = row[0]; tgt[i] = row[1:]
    if len(kfs) < 2: return None, unknown
    ko = np.array([k.offset for k in kfs])
    ours = np.stack([np.interp(offs, ko, [getattr(k, a) for k in kfs]) for a in ("openness", "width", "rounding")], 1)
    ...
    for j, name in enumerate(("openness", "width", "rounding")):
        ...
        m[f"{name}_r"] = float(np.corrcoef(a, b)[0, 1]) if a.std() > 1e-6 and b.std() > 1e-6 else float("nan")
    # lag via cross-correlation of openness (positive = ours late), hops
    a = ours[:, 0] - ours[:, 0].mean(); b = tgt[:, 0] - tgt[:, 0].mean()
    best, bestc = 0, -1e9
    for lag in range(-8, 9):
        if lag >= 0: c = float(np.dot(a[lag:], b[:n-lag]))
        else: c = float(np.dot(a[:n+lag], b[-lag:]))
    ...
    m["closure_gt"] = len(gt_cl); m["closure_ours"] = len(our_cl)
    ...
    for s, e in gt_n:
        idx = (offs >= s) & (offs < e)
        if idx.any(): cov.append(float((ours[idx, 0] <= 0.2).mean()))
    m["nasal_segments"] = len(gt_n); m["nasal_closed_fraction"] = float(np.mean(cov)) if cov else float("nan")
    ...
    m["per_class"] = per
    m["keyframes"] = len(kfs); m["dur"] = float(offs[-1])
    return m, unknown

async def main(args):
    rate = init()
    rows = []; unknown_all = set(); per_class_acc = {}
    for voice, pitch in VOICES:
        if args.voices and voice not in args.voices.split(","): continue
        for sid, text in SENTENCES.items():
            if args.sentences and sid not in args.sentences.split(","): continue
            ...
            for c, (cnt, o, w, r) in m["per_class"].items():
                ...
                acc[0] += cnt; acc[1] += o*cnt; acc[2] += w*cnt; acc[3] += r*cnt
            rows.append((voice, sid, m))
            if args.verbose:
                print(f"{voice:10s} {sid:14s} oMAE {m['openness_mae']:.2f} r {m['openness_r']:.2f} | wMAE {m['width_mae']:.2f} r {m['width_r']:.2f} | rndMAE {m['rounding_mae']:.2f} r {m['rounding_r']:.2f} | lag {m['lag_hops']:+d} | clos gt/ours {m['closure_gt']}/{m['closure_ours']} P {m['closure_precision']:.2f} R {m['closure_recall']:.2f} other {m['closure_on_other_stops']} | bilab open {m['openness_mean_on_bilabial']:.2f} | nasal closed {m['nasal_closed_fraction']:.2f} ({m['nasal_segments']} seg, {m['nasal_events']} ev) | kf/s {m['keyframes']/m['dur']:.1f}")
    def agg(k):
        v = np.array([r[2][k] for r in rows], dtype=float); return np.nanmean(v)
    print("\nAGGREGATE over", len(rows), "clips")
    for k in ("openness_mae", "openness_r", "width_mae", "width_r", "rounding_mae", "rounding_r", "lag_hops", "closure_precision", "closure_recall", "openness_mean_on_bilabial", "nasal_closed_fraction"):
        print(f"  {k:28s} {agg(k):.3f}")
    print(f"  closures: gt {sum(r[2]['closure_gt'] for r in rows)} ours {sum(r[2]['closure_ours'] for r in rows)} on-other-stops {sum(r[2]['closure_on_other_stops'] for r in rows)}")
    print("\nPER-CLASS mean of OUR (openness, width, rounding)  [target in brackets]")
    tgt_by_class = {}
    for sym, (c, o, w, r) in T.items(): tgt_by_class.setdefault(c, (o, w, r))
    tgt_by_class["SIL"] = SIL[1:]
    for c, (cnt, o, w, r) in sorted(per_class_acc.items(), key=lambda x: -x[1][0]):
        t = tgt_by_class.get(c, ("?",)*3)
        print(f"  {c:14s} n={cnt:5d}  open {o/cnt:.2f} [{t[0]}]  width {w/cnt:.2f} [{t[1]}]  round {r/cnt:.2f} [{t[2]}]")
    if unknown_all: print("\nunknown IPA symbols:", sorted(unknown_all))
    if args.json:
        json.dump([(v, s, {k: val for k, val in m.items() if k != 'per_class'}) for v, s, m in rows], open(args.json, "w"), indent=1)

if __name__ == "__main__":
    p = argparse.ArgumentParser(); p.add_argument("--voices"); p.add_argument("--sentences"); p.add_argument("-v", "--verbose", action="store_true"); p.add_argument("--json")
