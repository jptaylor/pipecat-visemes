...
class Tap(FrameProcessor):
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection):
        ...
        if isinstance(frame, TTSAudioRawFrame) and self.t0 is None:
            self.t0 = now
        if isinstance(frame, TTSLipsyncFrame):
            self.rows.append((frame.window_start, frame.window_end, frame.pts, now))
        await self.push_frame(frame, direction)


async def run(pacing_sleep, label, secs=3.0, params=None):
    pcm = np.concatenate(
        [synth_vowel(700, 1200, secs=secs / 2), synth_vowel(300, 2300, secs=secs / 2)]
    )
    frames = []
    for f in make_tts_frames(pcm, "ctx-1", chunk_ms=20):
        frames.append(f)
        if pacing_sleep and isinstance(f, TTSAudioRawFrame):
            frames.append(SleepFrame(sleep=pacing_sleep))
    proc = LipsyncProcessor(params=params or LipsyncParams())
    tap = Tap()
    ...
    print(
        f"{label:34s} batches={len(rows):2d} clamped={proc.stats['pts_clamped']:2d} | lead ms per batch (audio-time - emit-time): "
        + " ".join(f"{l:+.0f}" for l in leads[:8])
        + (" ..." if len(leads) > 8 else "")
    )
