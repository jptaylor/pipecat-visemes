...
| §4.2 events | 1-hop nasal entry on strong evidence, overriding the vowel trajectory to closed; silence after 300 ms | Silence after 200 ms (TTS pauses are 235–430 ms). Nasal: 2 consecutive hops; nasal features from the pre-emphasized spectrum; "missing F2 counts as damped" kept (needed on the Deepgram voice) but vetoed by any root at 500–1200 Hz above F1 (a back vowel's F2, however broad) and by F1 above 350 Hz (nasalized vowels); the override caps openness at 0.15 rather than forcing it shut | accuracy-improvements.md, deep-review-2026-09-results.md (third pass) |
| §4.1 rounding | F2 + F3 heuristic; low F2 with moderate F1 → rounded | F2 only, gated off for wide-open mouths (a low-side openness gate zeroed /u/); a broad F2-band root stands in when the strict slot is empty, and vetoed back vowels keep an openness of at least 0.1 | deep-review-2026-09-results.md (third pass) |
| §4.3 | P5/P95, 150 voiced frames to converge | P10/P90, 60 frames; adaptation spike rejection instead of an Hz median; a stale F1 drifts toward the range's low edge, not its center (a voiced frame with no findable F1 is a murmur or a close vowel) | accuracy-improvements-2.md; deep-review-2026-09-results.md (fourth pass) |
...
| §6.4 | batch at window close, 200 ms windows | keyframes and NASAL leave one hop (20 ms) after the window end; CLOSURE and SILENCE ride late in the next batch; the first window of an utterance is 100 ms; final keyframes are flushed after 100 ms without audio (mid-turn stall). The 0.4 s event-finalize horizon of 2026-07 held every batch and made the first one of each turn late. `flush()` also ends every utterance with one rest keyframe 100 ms past the last analyzed hop: audio stops at a turn end with no silence to decay through, so without it the last measured keyframe is mid-decay (0.45 openness on an /a/ with 80 ms of trailing silence) and the client holds that pose and eases out over about a second | README.md (delivery timing) |
...
| §11 | MFA phoneme truth, CI gates, a loopback timing rig, a performance harness | Praat + designed-corpus harness over seven Cartesia voices and one Deepgram voice, with committed baselines; delivery timing from the eval recorder; no CI, no performance harness | README.md |
